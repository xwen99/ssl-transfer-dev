...
class Birdsnap4Download(torch.utils.data.Dataset):
    ...
    def scrape_images(self, missing_ids, chunk_size=8196):
        ...
        def _load_url_and_save_image(idx, timeout):
            ...
            r = requests.get(self.meta[idx]['url'], stream=True, timeout=timeout, allow_redirects=True, verify=False)
...
class Birdsnap(torch.utils.data.Dataset):
    """ Birdsnap dataset """
    def __init__(self, root, split='train', transform=None, target_transform=None, download=False, loader=default_loader):
        self.transform = transform
        self.target_transform = target_transform
        self.path = root
        self.split = split
        ...
        if not self.split == 'test':
            with open(os.path.join(self.path, 'images_valid.txt'), 'r') as fp:
                reader = csv.DictReader(fp, delimiter='\t')
                for line in reader:
                    if os.path.exists(os.path.join(self.path, 'images', line['path'])):
            ...
            if self.split in ['train', 'val']:
                ...
                self.image_list = train_image if self.split == 'train' else val_image
                self.label_list = train_label if self.split == 'train' else val_label
        else:
            with open(os.path.join(self.path, 'test_images_valid.txt'), 'r') as fp:
...
if __name__ == '__main__':
    ...
    dataset = Birdsnap4Download(root='../datasets/birdsnap', split='test')
